...
final class MPVLayerRenderer {
    ...
    private func setupSubtitleFonts(_ handle: OpaquePointer) {
        ...
        // Hound does not yet bundle Streamyfin's Noto assets. Do not force
        // libass to select a family that is absent from its isolated font
        // database; retaining mpv's default selection preserves the behavior
        // of the previous module until those assets are added.
        if fm.fileExists(atPath: fontsDir.appendingPathComponent(Self.subtitleFontResources[0]).path) {
